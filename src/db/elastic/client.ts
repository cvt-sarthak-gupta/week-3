...
export class ElasticClient {
  ...
  readonly percolatorIndex = 'alert_percolator';
  ...
  async ensureIlmPolicies(): Promise<void> {
    for (const policy of TIER_POLICIES) {
      const body: Parameters<typeof this.client.ilm.putLifecycle>[0] = {
        name: policy.name,
        policy: {
          phases: {
            hot: {
      ...
      await this.client.ilm.putLifecycle(body);
  ...
  async ensureIndexTemplate(): Promise<void> {
    const body: Parameters<typeof this.client.indices.putIndexTemplate>[0] = {
      name: 'logs-template',
      index_patterns: ['logs-*'],
      priority: 500,
      template: {
    ...
    await this.client.indices.putIndexTemplate(body);
    logger.info('Elasticsearch index template "logs-template" ensured');
  }

  async ensureTemplateOnce(): Promise<void> {
   ...
   */
  async applyPolicyForProject(
    projectId: string,
    retentionDays: number,
  ): Promise<void> {
    await this.ensureTemplateOnce();
  ...
  async ensurePercolatorIndex(): Promise<void> {
    const exists = await this.client.indices.exists({ index: this.percolatorIndex });
    if (exists) {
      logger.info({ index: this.percolatorIndex }, 'Percolator index already exists, skipping creation');
      return;
    }

    await this.client.indices.create({
      index: this.percolatorIndex,
      mappings: {
        properties: {
