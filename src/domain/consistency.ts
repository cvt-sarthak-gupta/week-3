...
import { eventsCollection, projectConfigsCollection } from '../db/mongo.js';
...
async function auditProject(project: ProjectRow): Promise<Inconsistency[]> {
  const issues: Inconsistency[] = [];
  const { id: projectId, tenant_id: tenantId } = project;

  // (a) Check Mongo: project_configs document exists
  try {
    const mongoCount = await projectConfigsCollection().countDocuments({ _id: projectId });
    if (mongoCount === 0) {
      issues.push({
  ...
  try {
    ...
    const indexExists = await esClient.indices.exists({ index: pattern });
    if (!indexExists) {
      issues.push({
  ...
  try {
    const pgResult = await pool.query<{ total: string }>(
    ...
    const mongoTotal = await eventsCollection().countDocuments({ projectId });
...
export async function runAudit(): Promise<AuditResult> {
  ...
  const pgResult = await pool.query<ProjectRow>(
  ...
  const allIssues = (
    await runConcurrent(projects, 10, auditProject)
  ).flat();

  // 3. Check for orphaned Mongo configs (project_configs docs without a PG project)
  try {
    const projectIds = new Set(projects.map((p) => p.id));
    const mongoCursor = projectConfigsCollection().find(
      {},
      { projection: { _id: 1 } },
    );
