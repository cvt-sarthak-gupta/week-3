...
const alertPluginHandler: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  ...
  fastify.put<{ Params: AlertParams; Body: Partial<AlertRuleBody> }>(
    ...
    async (request, reply: FastifyReply): Promise<void> => {
      ...
      const existing = await pool.query<AlertRuleRow>(
  ...
  fastify.delete<{ Params: AlertParams }>(
    ...
    async (request, reply: FastifyReply): Promise<void> => {
      ...
      await esClient.delete({ index: percolatorIndex, id: alertId }).catch(() => {
...
export const alertRoutes = fp(alertPluginHandler, {
  name: 'alert-routes',
  fastify: '4.x',
});
