  ...
  async run(): Promise<void> {
    ...
    for (const { year, month } of months) {
      try {
        ...
        await this.pg.query(`SELECT ensure_billing_partition($1, $2)`, [year, month]);
