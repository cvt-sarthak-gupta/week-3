...
describe('E2: ILM Tier Policies', () => {
  beforeAll(async () => {
    const { ensureIlmPolicies } = await import('../../src/db/elastic.js')
  ...
  it('project requesting ≤30d retention maps to logs-tier-30d', async () => {
    const { resolveTierPolicy } = await import('../../src/db/elastic.js')
  ...
  it('project requesting 31–90d retention maps to logs-tier-90d', async () => {
    const { resolveTierPolicy } = await import('../../src/db/elastic.js')
  ...
  it('project requesting >90d retention maps to logs-tier-365d', async () => {
    const { resolveTierPolicy } = await import('../../src/db/elastic.js')
  ...
  it('applyPolicyForProject creates index and alias for the project', async () => {
    const { applyPolicyForProject } = await import('../../src/db/elastic.js')
